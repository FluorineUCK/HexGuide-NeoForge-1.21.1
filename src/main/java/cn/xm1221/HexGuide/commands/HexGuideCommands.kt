...
object HexGuideCommands {
    ...
    private val notesDir get() = FMLPaths.GAMEDIR.get().resolve("hexguide").resolve("notes")
    ...
    private fun exportNote(ctx: CommandContext<CommandSourceStack>): Int {
        ...
        if (!notes.isAllowed(player.uuid)) {
            ctx.source.sendFailure(Component.translatable("hexguide.command.no_authority"))
            return 0
        }
        val secs = notes.sections(player.uuid)
        if (index !in secs.indices) {
            ctx.source.sendFailure(Component.translatable("hexguide.command.index_out_of_bounds", secs.size, secs.size - 1))
            return 0
        }
        return try {
            ...
            ctx.source.sendSuccess({
                Component.translatable("hexguide.command.exported", index, secs[index].size, fileName)
            }, false)
            1
        } catch (e: Exception) {
            ctx.source.sendFailure(Component.translatable("hexguide.command.export_failed", e.message ?: e.javaClass.simpleName))
            0
        }
    }
    ...
    private fun importNote(ctx: CommandContext<CommandSourceStack>): Int {
        ...
        val file = safeImportPath(name)
        if (file == null) {
            ctx.source.sendFailure(Component.translatable("hexguide.command.invalid_filename"))
            return 0
        }
        if (!Files.exists(file)) {
            ctx.source.sendFailure(Component.translatable("hexguide.command.file_not_found", file.toString()))
            return 0
        }
        val iotas = try {
        ...
        if (iotas == null || iotas.isEmpty()) {
            ctx.source.sendFailure(Component.translatable("hexguide.command.invalid_file", name))
            return 0
        }
        notes.newSection(player.uuid, iotas)
        syncNotes(player, notes)
        ctx.source.sendSuccess({ Component.translatable("hexguide.command.imported", iotas.size, name) }, false)
        return 1
    }
    ...
    private fun setAuthority(ctx: CommandContext<CommandSourceStack>): Int {
        ...
        ctx.source.sendSuccess({
            Component.translatable(
                "hexguide.command.authority_set",
                target.name,
                Component.translatable(if (allowed) "hexguide.command.allowed" else "hexguide.command.denied")
            )
        }, true)
        return 1
    }
    ...
    /** Resolve an import without allowing absolute paths or traversal outside notesDir. */
    private fun safeImportPath(name: String): Path? {
        val root = notesDir.toAbsolutePath().normalize()
        val candidate = root.resolve(name).normalize()
        if (!candidate.startsWith(root) || !candidate.fileName.toString().endsWith(".json", ignoreCase = true)) {
            return null
        }
        return candidate
    }
