...
object DemoGenerator {
        ...
        for (iota in iotas) {
            ...
            when (iota) {
                is PatternIota -> {
                    ...
                    s.addProperty("push", IotaTextCodec.toPrefixed(iota))
                }
            }
            steps.add(s)
        }
     ...
     *
     * 保存位置与 [IotaTextCodec.saveToGameDir] 一致（游戏目录，不写入 mods jar 数据包），
    ...
    fun save(
        ...
        return try {
            ...
            val dir = FMLPaths.GAMEDIR.get().resolve(ns).resolve("spellplays")
