...
public abstract class MixinGuiSpellcasting implements BookSpellcastingAccess {
    ...
    @Shadow(remap = false) private List<Iota> cachedStack;
    ...
    // 演示执行状态使用 pre39 的 CastingImage 原生 CODEC 透传，避免手工 NBT 字段漂移。
    @Unique private CastingImage demoImage$hexguide = new CastingImage();
    ...
    private boolean blockPacket$hexguide(IClientXplatAbstractions inst, CustomPacketPayload msg) {
        if (msg instanceof MsgNewSpellPatternC2S s) {
            ...
            if (writeMode$hexguide) {
                ...
                PatternIota patternIota = new PatternIota(s.pattern());
                CompoundTag rec = HexCodecCompat.serializeIota(patternIota);
                List<Iota> newStack = new ArrayList<>(cachedStack);
                newStack.add(patternIota);
    ...
    @Override public void setStackClear$hexguide() {
        ...
        demoImage$hexguide = new CastingImage();
        this.calculateIotaDisplays();
    }
    ...
    @Override public void demoPushIota$hexguide(CompoundTag iotaNbt) {
        Iota iota = HexCodecCompat.deserializeIota(iotaNbt, null);
        if (iota == null) return;
        List<Iota> newStack = new ArrayList<>(cachedStack);
        newStack.add(iota);
        cachedStack = newStack;
    ...
    @Override public CompoundTag demoGetImageNbt$hexguide() {
        CastingImage image = new CastingImage(
            TreeList.from(cachedStack),
            demoImage$hexguide.getParenCount(),
            demoImage$hexguide.getParenthesized(),
            demoImage$hexguide.getEscapeNext(),
            demoImage$hexguide.getSimulateNext(),
            demoImage$hexguide.getOpsConsumed(),
            demoImage$hexguide.getUserData().copy()
        );
        return HexCodecCompat.serializeCastingImage(image);
    }

    /** 执行结果：用返回的 CastingImage 更新本地栈显示 + 读回执行状态（供下次上传继续转义/内省） */
    @Override public void demoSetImageNbt$hexguide(CompoundTag imageNbt) {
        CastingImage image = HexCodecCompat.deserializeCastingImage(imageNbt);
        if (image == null) return;
        demoImage$hexguide = image;
        cachedStack = new ArrayList<>(image.getStack());
        this.calculateIotaDisplays();
    }
    ...
    @Override public List<CompoundTag> getStack$hexguide() {
        return cachedStack.stream().map(HexCodecCompat::serializeIota).toList();
    }
