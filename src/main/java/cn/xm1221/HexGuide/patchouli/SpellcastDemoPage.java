...
public class SpellcastDemoPage extends BookPage {
    ...
    private static CompoundTag resolveIotaString(String s) {
        try {
            ...
            // "null" → NullIota
            if (s.equals("null")) {
                return HexCodecCompat.serializeIota(new NullIota());
            }
            if (s.startsWith("double:")) {
                double d = Double.parseDouble(s.substring(7).trim());
                return HexCodecCompat.serializeIota(new DoubleIota(d));
            }
            if (s.startsWith("vec:{")) {
                ...
                return HexCodecCompat.serializeIota(new Vec3Iota(new Vec3(x, y, z)));
            }
            if (s.startsWith("pattern:")) {
                // "pattern:<action id>" → 已注册图案的 PatternIota
                ActionRegistryEntry entry = HexActions.REGISTRY.get(ResourceLocation.parse(s.substring(8).trim()));
                if (entry != null) return HexCodecCompat.serializeIota(new PatternIota(entry.prototype()));
            }
            if (s.startsWith("pattern{")) {
                ...
                return HexCodecCompat.serializeIota(new PatternIota(pat));
            }
            if (s.startsWith("pattern[")) {
            ...
            if (s.startsWith("iota:[")) {
                ...
                for (String part : splitTopLevel(inner)) {
                    ...
                    Iota decoded = HexCodecCompat.deserializeIota(t, null);
                    if (decoded == null) return null;
                    elems.add(decoded);
                }
                return HexCodecCompat.serializeIota(new ListIota(elems));
            }
            if (s.startsWith("iota:")) {
                IotaInlineData data = IotaInlineData.parse(s.substring(5));
                if (data != null) {
                    Iota iota = data.getOrDeserialize();
                    if (iota != null) return HexCodecCompat.serializeIota(iota);
                }
            }
        } catch (Exception ignored) {}
        return null;
    ...
    private static HexPattern resolvePattern(Step step) throws Exception {
        if (!step.action.isEmpty()) {
            ActionRegistryEntry entry = HexActions.REGISTRY.get(ResourceLocation.parse(step.action));
    ...
    private void doExecute(BookSpellcastingAccess access, Step step) {
        try {
            ...
            playSound(HexSounds.START_PATTERN.value());
    ...
    private void doPush(BookSpellcastingAccess access, Step step) {
        ...
        if (step.pushIota != null) {
            ...
            playSound(HexSounds.CAST_NORMAL.value());
        }
    }
    ...
    private void doClear(BookSpellcastingAccess access, Step step) {
        access.demoClearCanvas$hexguide();
        playSound(HexSounds.STAFF_RESET.value());
    }
    ...
    public void onExecResult(CompoundTag image, String resolutionType) {
        ...
        if ("ERRORED".equals(resolutionType)) {
            ...
            playSound(HexSounds.CAST_FAILURE.value());
            ...
            playSound(HexSounds.CAST_SPELL.value());
        }
    }
