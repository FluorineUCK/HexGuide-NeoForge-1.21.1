...
class NoteIndex : BookPage() {
    ...
    override fun render(graphics: GuiGraphics, mouseX: Int, mouseY: Int, pticks: Float) {
        ...
        if (sections.isEmpty()) {
            graphics.drawString(font, Component.translatable("hexguide.notes.empty").string, 4, LIST_Y, META_COLOR, false)
        } else {
            ...
            for (i in 0 until visible) {
                val sec = sections[i]
                val title = sec.firstOrNull()?.title?.ifEmpty { Component.translatable("hexguide.notes.unnamed").string }
                    ?: Component.translatable("hexguide.notes.empty_section").string
            ...
            if (sections.size > MAX_ROWS) {
                graphics.drawString(font, Component.translatable("hexguide.notes.more", sections.size - MAX_ROWS).string,
                    4, LIST_Y + MAX_ROWS * ROW_H, META_COLOR, false)
            }
        }
    }
