...
object ScryingBookOverlay {
    ...
    fun registerSlateOverlay() {
        try {
            ScryingLensOverlayRegistry.addDisplayer(HexBlocks.SLATE.get(), ScryingLensOverlayRegistry.OverlayBuilder overlay@{ lines, _, pos, _, world, _ ->
                val tile = world.getBlockEntity(pos)
                if (tile is BlockEntitySlate) {
                    val pattern = tile.pattern ?: return@overlay
                    val lookupResult = PatternBookLookup.lookup(pattern)
                    // Line 1: pattern name
                    if (lookupResult != null && lookupResult.actionName != null) {
                        lines.add(Pair.of(
                            ItemStack(HexBlocks.SLATE.get().asItem()),
    ...
    fun onHudRender(graphics: GuiGraphics, partialTicks: Float) {
        try {
            ...
            // Keybinding check
            while (HexGuideKeybinds.OPEN_HEXBOOK.consumeClick()) {
